...
void CAGButton::EnableRoundCorner(BOOL bEnable)
{
    CRect rcButton;

    GetClientRect(&rcButton);

    if (m_rgnButton.GetSafeHandle() != NULL)
        m_rgnButton.DeleteObject();

    if (bEnable) 
        m_rgnButton.CreateRoundRectRgn(rcButton.left, rcButton.top, rcButton.right, rcButton.bottom, rcButton.Height(), rcButton.Height());

    m_bRoundCorner = bEnable;
    Invalidate(FALSE);
}
...
void CAGButton::DrawBack(LPDRAWITEMSTRUCT lpDrawItemStruct, UINT nStat)
{
    ...
    if (m_bRoundCorner)
        pDC->FillRgn(&m_rgnButton, &brushBack);
    else
...
void CAGButton::DrawFrame(LPDRAWITEMSTRUCT lpDrawItemStruct, UINT nStat)
{
    ...
    case AGBTN_NORMAL:
        ...
        if (m_bRoundCorner)
            pDC->FrameRgn(&m_rgnButton, &brushBorder, 1, 1);
        else
