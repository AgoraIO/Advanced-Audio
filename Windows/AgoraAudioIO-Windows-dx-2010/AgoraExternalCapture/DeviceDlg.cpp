...
void CDeviceDlg::OnShowWindow(BOOL bShow, UINT nStatus)
{
	...
	IRtcEngine *lpAgoraEngine = CAgoraObject::GetEngine();
	if (m_agPlayout.Create(lpAgoraEngine)){
	...
	if (m_agAudioin.Create(lpAgoraEngine)){
	...
	if (m_agCamera.Create(lpAgoraEngine)){
...
void CDeviceDlg::OnBnClickedBtnconfirmDevice()
{
	...
	//	if (!m_slkCamTest.IsWindowEnabled())
			//CAgoraObject::GetAgoraObject()->CaptureControl(TRUE);
	}
