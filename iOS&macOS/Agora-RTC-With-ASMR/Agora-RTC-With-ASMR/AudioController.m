                                ...
                                AudioBufferList *ioData)
{
    ...
    if (!status) {
        if ([audioController.delegate respondsToSelector:@selector(audioController:didCaptureData:bytesLength:)]) {
            [audioController.delegate audioController:audioController
                                       didCaptureData:(unsigned char *)bufferList.mBuffers[0].mData
                                          bytesLength:bufferList.mBuffers[0].mDataByteSize];
        }
    }
                               ...
                               AudioBufferList *ioData)
{
    ...
    if ([audioController.delegate respondsToSelector:@selector(audioController:didPlayData:bytesLength:)]) {
        result = [audioController.delegate audioController:audioController didPlayData:(uint8_t*)ioData->mBuffers[0].mData bytesLength:ioData->mBuffers[0].mDataByteSize];
    }
